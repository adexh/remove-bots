...
let state = {
  ...
  override: false,
  /** id -> true, tiles hidden from this user's own view. */
  hidden: {},
...
/** How many tiles are currently hidden from this user's view. */
export function hiddenCount() {
  return Object.keys(state.hidden).length;
}

/**
 * Hide the ticked bots' tiles from this user's own view.
 *
 * The consolation prize for a guest: it removes nobody and needs no
 * permission, it just stops drawing their tiles here. Ticks are left as they
 * are, so a later "Try anyway" still knows who was meant.
 */
export function hideSelected() {
  ...
  hide.apply(Object.keys(hidden));

  /* No notice here: the panel renders its own hidden-count line, with the
   * chip that undoes it, for as long as anything is hidden. */
  set({ hidden });
}

/** Show every hidden tile again. */
export function unhideTiles() {
  hide.apply([]);
  set({ hidden: {} });
}
...
export async function scan() {
  set({
     ...
     * do not carry over. Host status changes mid-call. Deliberately not
     * `hidden`: hiding is per-call, not per-scan, and a stale id for someone
     * who left matches nothing. */
    role: 'unknown',
    roleWhy: '',
    override: false,
  });
