...
/**
 * Every main-view video tile, roster open or not.
 *
 * Unlike tiles() above, this is not gated on the roster being closed: hiding a
 * tile has to work while the People panel is open, since that is where the
 * user is looking when they ask for it. Roster rows carry the same attribute,
 * so anything that is a listitem, or sits inside one, is excluded.
 */
function videoTiles() {
  return dom.dedupeBy(
    Array.prototype.filter.call(
...
/**
 * A stylesheet hiding those participants' main-view tiles, one rule per id.
 *
 * Hiding is done with CSS keyed on data-participant-id rather than by touching
 * the tile nodes, because Meet re-renders tiles and a fresh node keeps the
 * attribute, so the rule keeps applying where an inline style would be lost.
 * The :not() clauses keep the roster out of it: rows carry the same attribute
 * and must never be hidden.
 */
function hideTilesCss(ids) {
  return ids.map(function (id) {
    return '[data-participant-id="' + CSS.escape(id) + '"]' +
      ':not([role="listitem"]):not([role="listitem"] *) { display: none !important; }';
  }).join('\n');
}
...
export {
  SELECTORS, LABELS, rosterRows, tiles, videoTiles, hideTilesCss, findRow,
  participantName, isSelf, isVisitor,
