...
Everything about working on the extension: architecture, tests, fixtures, and
how releases ship. For what the extension does, see the [README](../README.md).
...
2. Select **`build/chrome-mv3-dev`**, not the repo root and not `build` itself.
   The root has no manifest: WXT generates one into each build folder.
...
All CI runs on PRs targeting `release`, and nowhere else: the full test suite
plus a production build (`.github/workflows/ci.yml`), and a CodeQL scan
(`.github/workflows/codeql.yml`). A merge into `release` can only carry green
code. Dependabot watches the npm trees and the workflow actions weekly.
...
| --- | --- | --- |
| `lib/bots.js` | Is this display name a bot? And is it yours (`ownedBy`)? Names only, no DOM. | nothing |
| `lib/dom.js` | Generic DOM helpers: waiting, clicking, accessible names. | nothing |
| `lib/meet.js` | **All Meet knowledge.** Selectors, label patterns, reading the roster, opening the panel, finding menu entries, tile CSS. | dom |
| `lib/engine.js` | Scan and removal logic. Holds no selectors. | bots, dom, meet |
| `lib/hide.js` | The injected stylesheet that hides bot tiles locally. | meet |
| `lib/store.js` | All UI state, held outside React so the service worker and the placement loop can drive it too. | engine, hide |
...
tables at the top of `lib/meet.js`.** Every non-obvious entry there records the
real-world detail that forced it. Change one, then run `pnpm test:dom` before
going near a live call.
...
- **Bot names are possessive.** Fathom joins as `Adesh's Fathom Notetaker`, not
  ...
  required. The same possessive is what `ownedBy()` reads to split Your bots
  from the rest.
